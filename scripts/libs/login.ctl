...
afterLogin(string user, string password, string newLocale, int closeModules = 1, int iConfNum = 1) 
{
  ...
  string panel    = "Boiler.pnl", s;           // default panelName  to be opened after login
  string module   = "WinCC_OA";                  // default moduleName to be used
  ...
  //Just Login and Close Login Dialog
  if ("-1" == closeModules)
     PanelOff();
  ...
  { 
    ...
    if (module == "")
      module = "WinCC_OA";              // default moduleName to be used 
    if (panel == "")
      panel  = "basePanel_user.pnl";   // default panel to be used 
  }
  //*********************************************************************************************
  
  if (dpExists("_"+getUserId(user)+"_UiConfiguration") || dpExists("_"+user+"_UiConfiguration"))
  { 
    useNewPT = true;
  }
  ...
  if (useNewPT)
  {
   ...
   //new feature 'userdependign alarm display' use a userdepending color scheme
...
{
  string defaultLogin = "vision/login.pnl";
  string loginPanel;
  loginPanel = paCfgReadValueDflt(getPath(CONFIG_REL_PATH, "config"), "ui", "loginPanel", defaultLogin);
